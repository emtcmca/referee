# Referee — Build Direction (internal)

> **This is the internal build brief, not the project README.** It was written to direct the
> implementation and it is kept in the repository because it records the reasoning and the
> reconciliation history behind the decisions. The public README is at the repository root.
>
> It is a historical document. Where it disagrees with the code, the code is right.
...
- Every bracketed placeholder in `scope/07-submission-kit.md` resolved
...
- **R11 · unmeasured quantitative claims deleted.** "Roughly 500 submissions compete and the top 10
  win" had no source. It was replaced by a placeholder against the rules page, then deleted
  outright on 2026-09-03 when nothing in the build depended on it. The corresponding sweep of `07`
